...
// SUPPORTIVE time and date functions
	// GET time for ICS adjusted for unix
		function evo_get_adjusted_utc($unix, $sep= true){
			...
			$customoffset = !empty($opt['evo_time_offset'])? intval($opt['evo_time_offset']):0;
			/*
			...
			*/
			...
			$__unix = $unix - $offset + $customoffset;
...
/* ADD TO CALENDAR */
	function eventon_get_addgoogle_cal($object, $sUNIX= '', $eUNIX=''){
		// unix
			$sUNIX = !empty($sUNIX)? $sUNIX: $object->estart;
			$eUNIX = !empty($eUNIX)? $eUNIX: $object->eend;
			
		$location = (!empty($object->evals['evcal_location']))? urlencode($object->evals['evcal_location'][0]) : ''; 
		$start = evo_get_adjusted_utc($sUNIX);
		$end = evo_get_adjusted_utc($eUNIX);
...
/* version 2.2.25 */	
	...
	// @version 2.3.16
		function eventon_trash_past_events(){
			...
			date_default_timezone_set('UTC');
			//$rightnow = current_time('timestamp', true);
			$rightnow = time();
