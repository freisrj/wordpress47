						...
						case 'ev_location':
							...
							<div class='evcal_data_block_style1'>
								...
								<div class='evcal_db_data'>
									...
									<div class='evoselectfield_saved_data evo_dark_metabox_darker' style='display:<?php echo $evo_location_tax_id?'none':'';?>'>
									<?php
									...
									<!-- image -->
										...
										<div class='evo_metafield_image' style='padding-top:10px'>					
					                    	...
					                    	<p><i><?php _e('<b>NOTE:</b> If location description is available it will also show over the location image selected above.','eventon')?></i></p>
					                    </div>
									
									<?php if(!empty($evo_location_tax_id)):?>
						                <p style='padding-bottom:10px'><a class='evo_btn' href="<?php echo esc_url( get_edit_term_link( $evo_location_tax_id, 'event_location', 'ajde_events' ) );?>"><?php _e('Further Edit Location Data','eventon');?></a></p>
						            <?php endif;?>
									...
									<!-- Show location name over image -->
										<p class='yesno_row evo'>
											...
											<label for='evcal_name_over_img'><?php _e('Show location information over location image (If location image exist)','eventon')?></label>
							...
							?>
								...
								<div id='evcal_rep' class='evd'>
									...
									<div id='evo_editevent_repeatevents' class='evcalr_2 evo_repeat_options' style='display:<?php echo $display ?>'>
										...
										?>
											<div class='repeat_weekly_only repeat_section_extra'>
												<p class='repeat_by evcalr_2_p evo_rep_week' style='display:<?php echo $__display_weekly_mode;?>'>
											...
											<?php
												...
												if(!empty($ev_vals['repeat_intervals'])){								
													...
													foreach($repeat_times as $rt){
														$startUNIX = (int)$rt[0];
														$endUNIX = (int)$rt[1];
														echo '<li style="display:'.(($count==0 || $count>3)?'none':'block').'" class="'.($count==0?'initial':'').($count>3?' over':'').'"><span>'.__('from','eventon').'</span> '.date($date_format_string,$startUNIX).' <span class="e">End</span> '.date($date_format_string,$endUNIX).'<em alt="Delete">x</em>
