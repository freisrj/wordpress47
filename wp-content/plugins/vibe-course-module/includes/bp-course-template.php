...
Class BP_Course_Template{
	...
	function get_instructor($args=NULL){
		...
		if(isset($this->instructors[$instructor_id])){
			return $this->instructors[$instructor_id];
		}
		...
		$instructor .='</div>';
		$this->instructors[$instructor_id] = apply_filters('wplms_display_course_instructor',$instructor,get_the_ID());
		return $this->instructors[$instructor_id];
	}
	 ...
	 */
	function get_students_undertaking($course_id){
		...
		if(empty($course_id)){
			...
			if(!empty($origin) && $origin == 'S'){
				...
				if(empty($dynamic) || $dynamic == 'H'){ // STATIC DRIP
					...
					foreach($curriculum as $i=>$c){
						if($i < $key && bp_course_get_post_type($c) == 'unit'){
							$unit_duration = get_post_meta($c,'vibe_duration',true);
							$unit_duration_parameter = apply_filters('vibe_unit_duration_parameter',60,$c);
							$addon_duration +=  $unit_duration*$unit_duration_parameter;
						}
					}
				}
...
if(!function_exists('bp_course_get_students_undertaking')){
	function bp_course_get_students_undertaking($course_id=NULL, $page=0){ // Modified function, counts total number of students
		...
		$template = BP_Course_Template::init();
		return $template->get_students_undertaking($course_id);
	}
}
...
function bp_course_remove_quiz_questions($quiz_id,$user_id){
	...
	global $wpdb;
	$wpdb->query($wpdb->prepare("UPDATE $wpdb->comments SET comment_approved='trash' WHERE comment_post_ID IN %s AND user_id=%d",$qs,$user_id));
...
if(!function_exists('bp_course_get_curriculum')){
	function bp_course_get_curriculum($course_id = NULL){
		...
		if(empty($course_curriculum))
			$course_curriculum= get_post_meta($course_id,'vibe_course_curriculum',true);
...
function bp_course_get_course_applicants_count($course_id){
	...
	$count = $wpdb->get_var("SELECT count(user_id) FROM {$wpdb->usermeta} WHERE meta_key = 'apply_course' AND meta_value = $course_id");
...
function bp_course_get_course_duration($course_id = NULL,$user_id = NULL){
	...
	if(empty($duration)){
		...
		$duration = $set_duration*$course_duration_parameter;
	}
			...
			if(!empty($question['content'])){
				echo '<li>
					...
					if($question['type'] == 'survey'){
						echo '<strong>'.__('Marked Choice :','vibe').'</strong> '.apply_filters('the_content',$question['marked_answer']).'<span>'._x('Score','survery question score awarded','vibe').' : '.$question['marks'].'</span>';
						...
						echo '<strong>'.__('Marked Answer :','vibe').'</strong> '.apply_filters('the_content',$question['marked_answer']);
					...
					if($type == 'survey'){
					    ...
					    switch($type){
					      	case 'truefalse': 
					        	...
					        	$user_result[$question]['marked_answer'] = apply_filters('the_content',$options[(intval($user_marked_answer))]);
