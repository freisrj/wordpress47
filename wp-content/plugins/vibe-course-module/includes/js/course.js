                             ...
                             var i = localStorage.length;
                              while(i--) {
                                ...
                                if($.isNumeric(key)) {
                                  ...
                                  localStorage.removeItem('question_result_'+key);
                                }  
                              }
...
function wplms_submit_quiz(quiz_id,security){
    ...
    if(all_questions_json.length){
      ...
      save_question_answer(current,defferred,per,$('.submit_quiz_progress'));
    }else{
       end_quiz_submission(quiz_id);
    }

    $('body').on('all_question_answers_saved',function(){
        end_quiz_submission(quiz_id);
    });
}
...
function wplms_submit_inquiz(){
    ...
    $('body').on('all_question_answers_saved',function(event){
        end_inquiz_submission();
    });
}
...
function save_question_answer(current,defferred,per,$this){
    ...
    if(current < defferred.length && all_questions_json.length >= per){
        ...
        $.ajax({
            ...
            success: function(){ 
                ...
                if(all_questions_json.length == per){
                    $('body').trigger('all_question_answers_saved');
                }else{
                    current++;
                    save_question_answer(current,defferred,per,$this);
                }
            }
        });
    }
...
jQuery(document).ready(function($){
   ...
   jQuery('.unit_description_expander').on('click',function(){
          ...
          case 'fillblank':
            ...
            if($this.find('input.ques').length){
              ...
              $this.find('.vibe_fillblank').each(function(){
                if(value){value+='|';}
                value += $this.find('.vibe_fillblank').text();
              });
            }
