...
function get_the_course_details($args=NULL){
  ...
  $course_details = array(
    'price' => '<li class="course_price">'.bp_course_get_course_credits('course_id='.$course_id).'</li>',
            ...
            foreach($questions['ques'] as $key=>$question){ // Grab all the Questions
                ...
                if(isset($question) && $question){
                    ...
                    if($type == 'survey'){
                        ...
                        $check = bp_course_evaluate_question(array('question_id'=>$question,'type'=>$type,'correct_answer'=>$correct_answer,'marked_answer'=>$user_marked_answer));

                        if($check){
                            $marks= apply_filters('wplms_correct_quiz_answer',$questions['marks'][$key],$quiz_id,$user_marked_answer,$question);
                            $total_marks = $total_marks+$marks;
