       ...
       global $wpdb,$bp;
        ...
        if($being > 3){
          ...
          echo '</h6>';
                ...
                $curriculum = apply_filters('wplms_course_student_stats',$curriculum,$course_id);
    			$curriculum .= '</ul></div>';
    	}
  ...
  function wplms_check_user_quiz_results($quiz_id = NULL,$user_id = NULL){
    ...
    */
    ...
    function search_course_members(){
        ...
        if(empty($s) && empty($active_status) && empty($course_status)){
            $query_tables ="FROM {$wpdb->users} as u LEFT JOIN {$wpdb->usermeta} as m ON u.ID= m.user_id";
            $query_where = "WHERE m.meta_key = $course_id";
        }
        ...
        if(empty($flag)){
            ...
            if(empty($user_ids)){
                ...
                echo '<li><div class="pagination"><div class="pag-count" id="course-member-count">'.sprintf(__('Viewing page %d of %d ','vibe'),(empty($page)?1:$page ),$max_page).'</div>';
          ...
          $quiztaken=get_user_meta($user_id,$id,true);
           ...
           if(!isset($quiztaken) || !$quiztaken){
            ...
            if($quiztaken > time()){
              ...
              <script>jQuery(document).reafy(function(){jQuery(".quiz_timer").trigger("end");});</script>';
            }
...
// After begin quiz, get unmarked quesitons
    ...
    function incourse_start_quiz(){
      ...
      if ( $the_query->have_posts() ) {
        echo '<script>var all_progress_json = all_questions_json = '.json_encode($questions).'</script>';
