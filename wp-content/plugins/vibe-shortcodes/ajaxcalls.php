        ...
        if(empty($flag)){
                    ...
                    if($setting->id == 'signup_username'){
                        ...
                        if(strpos($setting->id,'field') !== false){
                            ...
                            if(isset($form_settings[$setting->id])){
                                ...
                                if(in_array($form_settings[$setting->id],array('default_role'))){
            ...
            */
            ...
            if(empty($user_args['user_email']) || empty($user_args['user_pass']) || empty($check_filter)){
                echo '<div class="message">'._x('Invalid Email/Password !','error message when registration form is empty','vibe-shortcodes').'</div>';
                die();
            }
                    ...
                    foreach ( (array) $profile_field_ids as $field_id ) {
                        if ( !isset( $user_fields[$field_id] ) ) {
                            if ( !empty( $user_fields['field_' . $field_id . '_day'] ) && !empty( $user_fields['field_' . $field_id . '_month'] ) && !empty( $user_fields['field_' . $field_id . '_year'] ) )
                                $user_fields['field_' . $field_id] = date( 'Y-m-d H:i:s', strtotime( $_POST['field_' . $field_id . '_day'] . $user_fields['field_' . $field_id . '_month'] . $user_fields['field_' . $field_id . '_year'] ) );
                        }
            ...
            */
            ...
            if(empty($form_settings['skip_mail'])){
                ...
                if ( ! is_wp_error( $user_id ) ) {
                    ...
                    echo '<div class="message success">'.__('Congratulations ! you have been successfully registered !','vibe-shortcodes').'</div>';
                }else{
                    echo '<div class="message">'.$user_id->get_error_message().'</div>';
                }
            }else{
                $usermeta = array();
                ...
                if ( ! is_wp_error( $user_id ) ) {
                    ...
                    echo '<div class="message success">'.__('Congratulations ! you have been successfully registered, Please check your email to activate the account','vibe-shortcodes').'</div>';
            ...
            if(empty($form_settings['auto_login']) && !empty($user_id) && ! is_wp_error( $user_id )){
                if(!is_wp_error($user_id)){
                    ...
                    do_action( 'wp_login', $user_args['user_login'] ); 
