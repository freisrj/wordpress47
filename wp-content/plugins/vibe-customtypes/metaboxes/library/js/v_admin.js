(function($){
    ...
    $(function() {
        ...
        // Handles WPAUTOP based on tinymce configuration
        function switchEditorsAutoP(editorId, content, defaultWpAutoP) {
            var mceWpAutoP = ( tinyMCEPreInit && tinyMCEPreInit.mceInit )
                                ? !! ( ( tinyMCEPreInit.mceInit[ editorId ] || {} )[ 'wpautop' ] || !!defaultWpAutoP )
                                : !!defaultWpAutoP;
            if( mceWpAutoP ) {
                // AutoP is set to be processed. No special handling needed.
                content = switchEditors.wpautop( content );
            } else {
                // Autop handler
                var afterWpautopHandler = function ( e, obj ) {
                    obj.data = obj.unfiltered;
                };
                
                // Unique id for event namespace
                var uniqueEventId = new Date().getTime().toString();
                
                // Hook
                $( 'body' ).on( 'afterWpautop.' + uniqueEventId, afterWpautopHandler );
                
                // Process content
                content = switchEditors.wpautop( content );
                
                // Unhook
                $( 'body' ).off( 'afterWpautop.' + uniqueEventId );
            }
            return content;
        }
        ...
        $( 'body' ).delegate( 'span.settings_arrow', 'click', function(){
            ...
            $.ajax({
               ...
               $( 'body' ).delegate( '#generated_shortcode', 'click', function(){
                  $('#pagebuilder_generated_shortcode').toggle(200);
               });
            ...
            $('form#module_settings .v_option').each( function(){
                ...
                option_class = this_option_id + ' module_setting';
                 ...
                 if ( $(this).hasClass('v_wp_editor') ){
                    option_value = $(this).is(':hidden') ? tinyMCE.get( this_option_id ).getContent() : switchEditorsAutoP( this_option_id, tinymce.DOM.get( this_option_id ).value );                    
        ...
        $('#v_main_save').click(function(){
            ...
            $('#generated_shortcode').fadeIn(100);
            return false;
        });
        ...
        $( 'body' ).delegate( '.module', 'click', function(){
            ...
            $('#generated_shortcode').fadeOut(100);
        });
            ...
            $('.slides').sortable({
                ...
                stop: function(e,ui) {
                    $(this).find('.v_wp_editor').each(function(){
                        ...
                        init_new_editor( editor_id );
                        tinyMCE.execCommand( 'mceAddEditor', false, editor_id );
                        tinyMCE.execCommand( 'mceSetContent', false, switchEditorsAutoP( editor_id, $(this).val() ) );
        ...
        function init_new_editor(editor_id){
            ...
            var new_editor_object = jQuery.extend({}, hidden_editor_object, {
                elements: editor_id,
                selector: '#' + editor_id
            });
        ...
        function v_generate_layout_shortcode( html_element ){
            ...
            $('#pagebuilder_generated_shortcode').val(shortcode_output);
...
jQuery(document).ready(function($){ 
        ...
        var VibeBuilderPlugin = function() {
            YoastSEO.app.registerPlugin( 'VibeBuilderPlugin', {status: 'ready'} );
           ...
           */
          YoastSEO.app.registerModification( 'content', this.myContentModification, 'VibeBuilderPlugin', 5 );
        }
         ...
         */
        VibeBuilderPlugin.prototype.myContentModification = function(data) {
          return data + $('#pagebuilder_generated_shortcode').val();
        };
        
             new VibeBuilderPlugin();
        });
});
}());