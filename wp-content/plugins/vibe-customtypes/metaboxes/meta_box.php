					...
					switch( $type ) {
						...
						case 'dynamic_quiz_questions':
								...
								if(!empty($meta['tags']) && !empty($meta['numbers'])){
									...
									foreach( $tags as $i=>$tag ) {
										...
										echo '<strong>'.$tags_string. '</strong>
											  	<input type="number" name="' . $field['id'] . '[numbers]['.$i.']" placeholder="'.__('Number of questions to pull','vibe-customtypes').'" value="'.esc_attr( $numbers[$i] ).'"/>
							...
							echo '<li class="hide">
								  ...
								  echo '</select>
								 <input type="number" rel-name="' . $field['id'] . '[numbers]" placeholder="'.__('Number of questions to pull','vibe-customtypes').'" value="0" /> 
								 <a class="meta_box_repeatable_remove" href="#"><span class="dashicons dashicons-no"></span></a></li>';
							echo '</ul>

							    <strong>'.__('Total Question Count :','vibe-customtypes').' <span id="total_question_number"> '.$total_number.'</span></strong><br />
								<span class="description">' . $field['desc'] . '</span>';
						break;
						// slider
						case 'slider':
