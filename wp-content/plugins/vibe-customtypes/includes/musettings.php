	...
	*/
	...
	function scheduled_emails(){
		...
		if(!function_exists('_get_cron_array'))
			return;
		if(!empty($_POST) && isset($_POST['remove_schedule'])  && wp_verify_nonce($_POST['remove_scheduled_email_security'],'remove_scheduled_email_'.$_POST['timestamp'])){

			if($_POST['hook'] == 'wplms_send_course_expiry_mail'){
				if(empty($_POST['group']))
					$group = '';
				$original_args = array($_POST['course'],$_POST['user'],$_POST['group']);
				 $key = md5(serialize($original_args));
			}elseif($_POST['hook'] == 'wplms_send_drip_mail'){
				$original_args = array($_POST['unit'],$_POST['course'],$_POST['user']);
				 $key = $_POST['cron_key'];
			}else{
				echo 0;
			}
			...
			foreach($crons as $timestamp => $cron){
				if(isset($cron['wplms_send_course_expiry_mail']) || isset($cron['wplms_send_drip_mail'])){
					...
					if(isset($cron['wplms_send_course_expiry_mail'])){
						...
						foreach($value as $v){
							$data .= '<input name="user" type="hidden" value="'.$v['args'][1].'" data-user="'. $v['args'][1].'"> ';
							$data .= '<input name="group" type="hidden" value="'.$v['args'][2].'" data-group="'. $v['args'][2].'"> ';
							$data .= '<input name="course" type="hidden" value="'.$v['args'][0].'" data-course="'. $v['args'][0].'"> ';
						...
						foreach($value as $v){
							...
							$data .= '<input name="user" type="hidden" value="'.$v['args'][2].'" data-user="'. $v['args'][2].'"> ';
							$data .= '<input name="unit" type="hidden" value="'.$v['args'][0].'" data-unit="'. $v['args'][0].'"> ';
							$data .= '<input name="course" type="hidden" value="'.$v['args'][1].'" data-course="'. $v['args'][1].'"> ';
							$data .= '<input name="timestamp" type="hidden" value="'.$timestamp.'">';
